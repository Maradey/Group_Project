...
class Settlement: public Region//Ianaeaiee ioieo
{
    string settlementName; //iacaa
    string geomorphicBinding; //aaiii?oieia??ia i?ea’ycea i?noy a?aai?o i?iae
...
class SamplingData //Aai? a?aai?o i?ia
{
    Settlement settlement;//Ianaeaiee ioieo
    string samplingDate; //aaoa a?aai?o i?iae
    double CO2Content; //ai?no aoaeaeeneiai aaco
    double COContent; //ai?no NI
    double SO2Content; //ai?no SO2
    double GasolineContent; //ai?no aaiceio
    double HClContent; //ai?no HCl
...
//Eeann aey caieno aaieo i?ia ai oaeeo oa eiai ?eoaiiy
class SampleStorage{
    vector<SamplingData> samples;
public:
    SampleStorage();
    void addSample(); //aiaaoe i?iae
    void deleteSample(); //aeaaeeoe i?iae
    void searchSample(); //ciaoe i?iao
    void viewSamples(); //a?aia?aceoe i?iae
    void fullTable();
private:
    void readSamplesFromFile(); //?eoaoe an? i?iae c oaeeo
    void writeSamplesToFile(); //cainaoe an? i?iae ai oaeeo
    void printSample(int i); //Aeaia no?iee i?iae ai ae?aio
...
int main()
{
    ...
    SampleStorage storage; //noiaeua aey aaieo i?ia
